...
//! AID (AI Config) bootstrapper — project-level VISION.md + FEATURES.md stubs.
//!
//! Covers PO.T13–PO.T15: detect missing `.claude/` directory, offer bootstrap,
...
/// Returns `true` if the repo at `repo_path` has NO `.claude/` directory —
...
/// ```text
/// .claude/
///   CLAUDE.md          — brief repo-specific instructions stub
///   docs/
///     VISION.md        — product vision stub
///     FEATURES.md      — feature inventory stub
///   tasks/
///     active.md        — task tracking stub
///   memory/
///     decisions.md     — architecture decision log stub
/// ```
///
...
[TODO: One sentence — what is the ultimate goal of this project?]
...
| # | Task | Status | Weight |
|---|------|--------|--------|
| T01 | Fill in VISION.md | 🔲 planned | XS |
| T02 | Fill in FEATURES.md | 🔲 planned | XS |
| T03 | Add project-specific rules to CLAUDE.md | 🔲 planned | XS |
...
fn render_decisions_stub(repo_name: &str) -> String {
    let date = Utc::now().format("%Y-%m-%d").to_string();
    format!(
        r#"# {repo_name} — Architecture Decisions
...
*(None yet — add your first decision when you make one)*
...
async fn write_if_missing(path: &Path, content: &str) -> Result<()> {
    if path.exists() {
        debug!(path = %path.display(), "skipping — file already exists");
        return Ok(());
    }
