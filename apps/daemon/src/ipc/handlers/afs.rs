...
///
/// Params: `{ path: String, stack?: String }`
/// If `stack` is omitted, the stack is auto-detected from marker files.
pub async fn init(params: Value, _ctx: &AppContext) -> Result<Value> {
    use crate::init_templates::{detect_stack, template_for, Stack};
    ...
    // Stack detection or explicit override.
    let stack = if let Some(s) = params.get("stack").and_then(|v| v.as_str()) {
        s.parse::<Stack>().unwrap_or(Stack::Generic)
    } else {
        detect_stack(path_obj)
    };
    ...
    if gitignore.exists() {
        ...
        let missing_stack = !tmpl.gitignore_additions.trim().is_empty()
    ...
    Ok(json!({ "path": path, "stack": stack.as_str(), "created": created, "ok": true }))
}
